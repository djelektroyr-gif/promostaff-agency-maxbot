...
### 2.3. Следующий продуктовый шаг (после wave 1, без перепрыгивания на отчёты)

- Контур формирования проекта заказчиком (создание, лимиты/количество, связь с подпиской).
- Контур подписки заказчика (оформление, статус, дата истечения, контроль/алерты/продление).
- Контур «Команда заказчика» (добавление своего персонала заказчиком и привязка к проектам/сменам).
