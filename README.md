...
- Визитка: сценарий перенесён с **PROMOSTAFF-AGENCY BOT** (`keyboards.py` + тексты/FSM из `handlers.py`): главное меню, разделы, расчёт (многоэкранный сценарий), вопрос, анкета в команду (включая выбор должности). Контакты и ссылки — из `config` (см. `.env.example`, дефолты как на рабочем столе).  
